...
const page = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (response.ok) {
                setPending(false)
                setPortfolio({ ...service, image: data.url });
            } else {
    ...
    const handleContentChange = (content) => {
        setPortfolio({ ...service, description: content });
    };
    return (
        <div className='bg-gray-900 w-full p-5 px-10'>
            <h3 className='text-4xl text-white font-bold py-5 text-left merriweather'>Create Portfolio</h3>
           ...
           <div className="flex flex-col justify-center items-center gap-y-1 py-2"> 
            ...
            <form onSubmit={(e) => createPortfolio(e)} className='w-full md:w-2/3 py-5 mx-auto'>
                ...
                <input className='w-full text-white bg-transparent border-2 border-secondary-600 rounded-md mb-4 focus:shadow-md p-2 focus:shadow-secondary-400' type="text" name='slug' value={portfolio.slug} onChange={(e) => setPortfolio({ ...service, [e.target.name]: e.target.value })} />
