...
const page = ({params}) => {
    ...
    const createService = async (e) => {
        e.preventDefault()
        const res = await axios.put('/api/service', service, {
            headers: {
    ...
    return (
        <div className='bg-gray-900 w-full p-5 px-10'>
            <h3 className='text-4xl text-white font-bold py-5 text-left merriweather'>Update Service</h3>
            {isLoading && (<Loading />)}
           ...
           <div className="flex flex-col justify-center items-center gap-y-1 py-2"> 
            ...
            <form onSubmit={(e) => createService(e)} className='w-full md:w-2/3 py-5 mx-auto'>
                ...
                <button type="submit" className="bg-secondary-500 text-white px-3 py-2 rounded-md">Update Service</button>
            </form>
            </>}
        </div>
    )
